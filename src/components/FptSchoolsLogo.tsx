...
interface FptSchoolsLogoProps {
  className?: string;
  height?: number | string;
}

/**
 * Official FPT Schools Vector Logo
 * Features:
 * - 100% authentic geometry of the 3 iconic FPT parallelograms with curved ends & tilted letters F, P, T
 * - Official brand colors: FPT Blue (#08509F), FPT Orange (#F27123), FPT Green (#51B748)
 * - Registered trademark ® symbol
 * - Official bold slab-serif typography for "FPT SCHOOLS"
 * - 100% transparent background (no white box)
 */
export const FptSchoolsLogo: React.FC<FptSchoolsLogoProps> = ({
  className = 'h-8 sm:h-9 w-auto',
  height
}) => {
  return (
    <div
      className={`inline-flex items-center justify-center select-none transition-transform duration-200 hover:scale-105 ${className}`}
      style={height ? { height } : undefined}
      title="FPT Schools"
    >
      <svg
        viewBox="0 0 160 76"
        className="h-full w-auto max-h-14 drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          <style>{`
            @import url('https://fonts.googleapis.com/css2?family=Alfa+Slab+One&family=Rokkitt:wght@900&display=swap');
            .fpt-schools-text {
              font-family: 'Rokkitt', 'Alfa Slab One', 'Rockwell', 'Clarendon', 'Georgia', serif;
              font-weight: 900;
        ...
        {/* Top: Official FPT 3-Rhombus Emblem with White Slanted F, P, T */}
        <g id="fpt-official-emblem" transform="translate(48.5, 4) scale(1.85)">
          {/* Blue Rhombus */}
          <path
          ...
          {/* Registered Trademark ® in Blue */}
          <circle cx="32.5" cy="15.8" r="1.4" stroke="#08509F" strokeWidth="0.35" fill="none" />
          <text
            x="32.5"
            y="16.5"
            fill="#08509F"
        ...
        {/* Bottom Text: FPT SCHOOLS in Authentic Heavy Slab Serif */}
        <text
          x="80"
          y="68"
          textAnchor="middle"
